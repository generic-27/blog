...
<div class="blog-text-container">
	...
	<div class="blog-paragraph">
		The first part of this series <strong>(You don't know JS)</strong> was a general introduction to
		programming and a discussion of some aspects of <i>JavaScript</i>. There are few topics that
		caught my attention.
		<a
	...
	<div class="blog-paragraph">
		...
		<p>
			The conversion in the above cases is explicit. We know that the <code>name</code> variable is
			a <code>string</code> and so is the variable <code>numString</code>. In the third case the
		...
		<p>
			The conversion in the first case is explicit, we know that num is a string. In the second case
			the value of num is coerced into a number.
		</p>
	</div>
	<div class="blog-sub-header">Typed values not typed variables</div>
	<div class="blog-paragraph">
		<p>
			There is a neat way to learn the type of a variable in JavaScript. It's the typeof() operator.
			When checking the type of a variable, JavaScript looks at the value of the variable and not
			the variable itself. The variables in JavaScript are merely containers of <code>types</code>.
		</p>
	</div>
	<div class="blog-sub-header">Equality</div>
	<div class="blog-paragraph">
		<p>
			Equality in JavaScript comes in two flavors <code>==</code> and <code>===</code>. The easiest
			way to understand these would be that <code>==</code> checks for equality with coercion and
	...
	<div class="blog-paragraph">
		<p>
			On declaring a variable in JavaScript, it's made available throughout the entire scope it was
			declared in. JavaScript hoists that variable to the top, to be accessed throughout. The same
			works for functions. Variable hoisting can cause a lot of confusion and it's better to avoid
			it. The best approach is to use JavaScript in "strict" mode and use let for variable
			declaration.
		</p>
		<div class="blog-code-block">{@html marked(hoisting)}</div>
		<p>
			In the above example, on calling the function <code>hoistedVariable</code> we notice that the
			variable until initialized prints a value of <code>undefined</code>. The declaration of the
			variable
	...
	<div class="blog-paragraph">
		<p>
			One can think of Closure as a way to "remember" a function's scope, like variables, even after
			the function has finished executing. The best way to explain closure is through an example
		</p>
	...
	<div class="blog-paragraph">
		<p>
			...
			that's equivalent to the behavior, but is able to run in older JavaScript environments. As an
			example, ES6 deprecated the utility <code>isNan(...)</code> which was buggy and replaced it
			with
